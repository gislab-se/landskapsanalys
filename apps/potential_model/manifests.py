...
def _package_region_manifest_paths() -> list[Path]:
    root = regions_root()
    if not root.exists():
        return []
    return sorted(root.glob("*/region.json"))


def _legacy_region_manifest_paths() -> list[Path]:
...
def list_region_manifest_paths() -> list[Path]:
    paths: list[Path] = []
    seen: set[str] = set()
    for path in [*_indexed_region_manifest_paths(), *_package_region_manifest_paths(), *_legacy_region_manifest_paths()]:
...
def load_region(region_id: str) -> dict[str, Any]:
    ...
    legacy_path = manifests_root() / "regions" / f"{region_id}.json"
    package_path = _region_manifest_path_from_index(str(region_id))
    raise FileNotFoundError(f"Region manifest not found: {package_path} or {legacy_path}")
