...
layer_specs <- list(
  cultural_preservation = list(
    layer_key = "trl_cultural_heritage",
    label = "Kulturmiljoer och kulturminnen",
    source_path = "D:/LABLAB_Energiforsk/Projekt SL01/Geodatakatalog_SL01/Utkommande_SL01/UT_Trondelag_SL01/UTM 32/RA_Kulturminner-Culture_NW_32.shp",
    note = "Norwegian RA cultural heritage environments. Mirrors Bornholm cultural_preservation as a culture hard-exclusion source."
  ),
  valuable_cultural_environment = list(
    layer_key = "trl_cultural_landscapes",
    label = "Vardefulla kulturlandskap",
    source_path = "D:/LABLAB_Energiforsk/Projekt SL01/Geodatakatalog_SL01/Utkommande_SL01/UT_Trondelag_SL01/UTM 32/NEA_CulturalLandscapes_Area_TRL_32.shp",
    note = "Norwegian NEA cultural landscapes. Mirrors Bornholm valuable_cultural_environment as a culture hard-exclusion source."
  )
)
...
for (layer_id in names(layer_specs)) {
  if (!file.exists(layer_specs[[layer_id]]$source_path)) {
    stop("Trondelag culture source not found for ", layer_id, ": ", layer_specs[[layer_id]]$source_path)
  }
}
...
for (layer_id in names(layer_specs)) {
  ...
  if (nrow(source_clipped) == 0) {
    stop("Culture layer contains no features after clipping to Trondelag landmask: ", layer_id)
  }
  ...
  analysis_geom <- suppressWarnings(
    st_simplify(st_sfc(analysis_geom, crs = working_epsg), dTolerance = 25, preserveTopology = TRUE)
  )
  analysis_sf <- st_sf(
    tibble(
      layer_id = layer_id,
  ...
  display_sf <- st_sf(
    tibble(
      ...
      tooltip_body = paste0(spec$note, "<br>Source clipped to Trondelag landmass.")
    ),
...
message("Wrote Trondelag culture assets to: ", asset_dir)
