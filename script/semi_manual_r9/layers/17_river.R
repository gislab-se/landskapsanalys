# Step 17 (Original layer 7): River split by midtebredd
# Explicit manual workflow:
# 1) Inspect input + width classes (default; no aggregation)
...
if (is.null(script_dir)) {
  if (file.exists("script/semi_manual_r9/layers/17_river.R")) {
    script_dir <- normalizePath("script/semi_manual_r9/layers", winslash = "/", mustWork = TRUE)
  } else if (file.exists("layers/17_river.R")) {
...
if (length(split_field) != 1) {
  stop("Expected exactly one split field for River, got: ", paste(split_field, collapse = ", "))
}
...
message("River split values in source layer:")
print(data.frame(
...
if (length(unexpected) > 0) {
  warning("Unexpected River split values found outside config: ", paste(unexpected, collapse = ", "))
}
...
if (do_mapview) {
  if (!requireNamespace("mapview", quietly = TRUE)) {
    ...
    message("Opening pre-aggregation mapview (River by midtebredd)...")
    pre_map <- mapview::mapview(
      ...
      layer.name = paste0("Input rivers (", split_field, ")")
    )
    print(pre_map)
  }
}
...
if (!run_aggregation) {
  ...
  rivers_m <- sf::st_transform(source_layer, metric_crs) %>% dplyr::select(split_slug)
  ix <- suppressWarnings(sf::st_intersection(rivers_m, hex_m))

  prefix <- layer_row$layer_key
  total_col <- paste0(prefix, "_length_m")
  ...
  if (nrow(ix) > 0) {
    ...
    tmp_total <- ag_total[, c("hex_id", "len_m"), drop = FALSE]
    ...
    for (i in seq_len(nrow(split_rows))) {
      ...
      tmp <- ag_split[ag_split$split_slug == slug_i, c("hex_id", "len_m"), drop = FALSE]
      names(tmp)[2] <- col_nm
  ...
  if (do_mapview) {
    if (!requireNamespace("mapview", quietly = TRUE)) {
      ...
      message("Opening post-aggregation mapview (River total length)...")
      ...
      post_map <- mapview::mapview(
        ...
        zcol = "split_value_raw",
        alpha.regions = 0.30,
        layer.name = paste0("Input rivers by ", split_field)
      )
      print(post_map)
    }
  }
