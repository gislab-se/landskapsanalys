...
selected_cols <- c(
  ...
  gc_roads_medium_length_m = "roads_simplified_gd_v_vej_road_merged_bol_33_length_m_medium",
  gc_roads_large_length_m = "roads_simplified_gd_v_vej_road_merged_bol_33_length_m_large",
  ...
  gc_fredskov_share = "fredskov_miljostyrelsen_gds_mat2_fredskov_bor_32_area_share",
  gc_agricultural_land_share = "markblokke_2026_bornholm_area_share",
  gc_relief_m = "dhm_hoejdekurver_relief_m",
  gc_protected_watercourses_length_m = "protected_watercourses_miljostyrelsen_length_m"
)

indicator_catalog <- tibble::tibble(
  gc_name = names(selected_cols),
  source_name = unname(selected_cols),
  display_name = c(
...
write.csv(indicator_catalog, file.path(out_dir, paste0(analysis_id, "_indicator_catalog.csv")), row.names = FALSE)

analysis_metadata <- tibble::tibble(
  analysis_id = analysis_id,
  analysis_title = "Landskapsanalys: Bornholm R9",
  analysis_subtitle = "9 lager, R9",
  archived_previous_version = "landskapsanalys_gc4_res9",
  n_input_layers = nrow(indicator_catalog),
  source_model_note = "GC4-logiken utokad med medel/stor vag, fredskov, jordbruksmark, relief och skyddade vattendrag"
)
...
pop_locations <- bind_cols(points, st_drop_geometry(hex) |> select(all_of(names(selected_cols)))) |>
...
points_with_context <- compute_context(points, pop_locations, groups = names(selected_cols))
