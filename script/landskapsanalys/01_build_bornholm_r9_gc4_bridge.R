...
out_dir <- file.path(repo_root, "data/interim/landskapsanalys_gc4_bridge")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

legacy_cols <- c(
  gc_fastboende_count = "fastboendebefolkningmapinfo_count",
  gc_roads_total_length_m = "roads_simplified_gd_v_vej_road_merged_bol_33_length_m_total",
  gc_ecology_connectivity_share = "ecology_connectivity_pdk_oekologiskforbindelse_bor_32_area_share",
  gc_cultural_historical_conservation_share = "cultural_and_historical_conservation_values_kulturhistoriske_bevaringsvardier_pdk_kulturhistoriskebevaringsvardier_bor_32_area_share"
)

indicator_catalog <- tibble::tibble(
  gc_name = names(legacy_cols),
  source_name = unname(legacy_cols),
  display_name = c(
  ...
  geometry_type = c("Point count", "Line length", "Polygon area share", "Polygon area share")
)
write.csv(indicator_catalog, file.path(out_dir, "indicator_catalog.csv"), row.names = FALSE)

message("Reading merged R9 matrix...")
raw_df <- read.csv(input_csv, check.names = FALSE)
selected <- raw_df |>
  select(hex_id, all_of(unname(legacy_cols)))
colnames(selected) <- c("hex_id", names(legacy_cols))
...
hex <- hex |>
  ...
  mutate(across(all_of(names(legacy_cols)), ~tidyr::replace_na(.x, 0)))
...
pop_locations <- bind_cols(points, st_drop_geometry(hex) |> select(all_of(names(legacy_cols)))) |>
  mutate(total = rowSums(across(all_of(names(legacy_cols))), na.rm = TRUE))
...
points_with_context <- compute_context(points, pop_locations, groups = names(legacy_cols))
write.csv(points_with_context, file.path(out_dir, "bornholm_r9_gc4_bridge_points_with_context.csv"), row.names = FALSE)
...
n_factors <- 5
set.seed(42)
fa_fit <- suppressWarnings(psych::fa(Xz_clean, nfactors = n_factors, rotate = "varimax", fm = "minres", scores = "tenBerge", warnings = FALSE))
...
write.csv(loadings_df, file.path(out_dir, "bornholm_r9_gc4_bridge_factor_loadings.csv"), row.names = FALSE)
...
write.csv(variance_df, file.path(out_dir, "bornholm_r9_gc4_bridge_factor_variance.csv"), row.names = FALSE)
...
write.csv(k_scores, file.path(out_dir, "bornholm_r9_gc4_bridge_k_scores.csv"), row.names = FALSE)
...
write.csv(factor_scores, file.path(out_dir, "bornholm_r9_gc4_bridge_factor_scores.csv"), row.names = FALSE)

cluster_profile <- factor_scores |>
  group_by(class_km) |>
  summarise(across(all_of(factor_names), ~mean(.x, na.rm = TRUE)), n_hex = dplyr::n(), .groups = "drop")
write.csv(cluster_profile, file.path(out_dir, "bornholm_r9_gc4_bridge_cluster_profile.csv"), row.names = FALSE)

cluster_sizes <- factor_scores |>
  count(class_km, name = "n_hex") |>
  mutate(share = n_hex / sum(n_hex))
write.csv(cluster_sizes, file.path(out_dir, "bornholm_r9_gc4_bridge_cluster_sizes.csv"), row.names = FALSE)

points_with_model <- points_with_context |>
  left_join(factor_scores, by = "hex_id")
write.csv(points_with_model, file.path(out_dir, "bornholm_r9_gc4_bridge_points_with_context_and_scores.csv"), row.names = FALSE)

hex_model <- hex |>
  left_join(points_with_model, by = c("hex_id", names(legacy_cols)))
out_gpkg <- file.path(out_dir, "bornholm_r9_gc4_bridge_hex.gpkg")
if (file.exists(out_gpkg)) invisible(file.remove(out_gpkg))
st_write(hex_model, out_gpkg, layer = "landskapsanalys_gc4_bridge", quiet = TRUE)
...
write.csv(run_summary, file.path(out_dir, "bornholm_r9_gc4_bridge_run_summary.csv"), row.names = FALSE)

message("Finished R9 GC4 bridge run.")
