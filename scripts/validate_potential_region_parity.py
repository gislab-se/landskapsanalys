...
WIND_POLYGON_REFERENCE_LABEL = "Landskapspotential Vind polygon"
...
def _check_region_contract(report: ParityReport, result: dict[str, Any], reference: dict[str, Any] | None = None) -> None:
    ...
    report.check(
        WIND_POLYGON_REFERENCE_LABEL not in result["layer_names"],
        f"{region_id}: wind polygon reference layer is removed from the map layer list.",
        f"{region_id}: removed wind polygon reference layer is still present; layers={result['layer_names']}.",
    )
